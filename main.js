...
function renderProducts(list) {
    ...
    list.forEach((product) => {
        ...
        <a href="product.html?id=${product.id} "class="view-details">View Detailes</a>
        </div>
        `;
...
function addTocart(id) {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    let quantities = JSON.perse(localStorage.getItem("quantities")) || {};
...
sortSelect.addEventListener("change", () => {
    ...
    if (value === "priceLow") {
        ...
        sorted.sort((a, b) => b.sold - a.sold); // sort by best selling
    } else if (value === "category") {
        sorted.sort((a, b) => a.category.localeCompare(b.category)); // Sort by category
    }

    // Re-render the products with sorted list
